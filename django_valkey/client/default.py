...
from valkey.typing import EncodableT, ExpiryT, AbsExpiryT, PatternT
...
class SyncClientMethod(BaseClient):
    ...
    def hgetall(
        self, name: str, client: Valkey | Any | None = None
    ) -> dict[str, str] | dict:
        client = self._get_client(write=False, client=client)
        try:
    ...
    def hvals(self, name: str, client: Valkey | Any | None = None) -> list:
        client = self._get_client(write=False, client=client)
        try:
...
class DefaultClient(SyncClientMethod):
    ...
    def connect(self, index: int = 0) -> Valkey | Any:
        return self.connection_factory.connect(self._server[index])

    def disconnect(self, index: int = 0, client: Valkey | Any | None = None) -> None:
        """
