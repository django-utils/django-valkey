...
class TestDjangoValkeyCache:
    def test_setnx(self, cache: ClusterValkeyCache):
    ...
    def test_setnx_timeout(self, cache: ClusterValkeyCache):
    ...
    def test_unicode_keys(self, cache: ClusterValkeyCache):
        cache.set("ключ", "value")
        res = cache.get("ключ")
        assert res == "value"

    def test_save_and_integer(self, cache: ClusterValkeyCache):
    ...
    def test_save_string(self, cache: ClusterValkeyCache):
    ...
    def test_save_unicode(self, cache: ClusterValkeyCache):
    ...
    def test_save_dict(self, cache: ClusterValkeyCache):
    ...
    def test_save_float(self, cache: ClusterValkeyCache):
    ...
    def test_timeout(self, cache: ClusterValkeyCache):
    ...
    def test_timeout_0(self, cache: ClusterValkeyCache):
        cache.set("test_key", 222, timeout=0)
        res = cache.get("test_key")
        assert res is None

    def test_timeout_parameter_as_positional_argument(self, cache: ClusterValkeyCache):
    ...
    def test_timeout_negative(self, cache: ClusterValkeyCache):
    ...
    def test_timeout_tiny(self, cache: ClusterValkeyCache):
        cache.set("test_key", 222, timeout=0.00001)
        res = cache.get("test_key")
        assert res in (None, 222)

    def test_set_add(self, cache: ClusterValkeyCache):
    ...
    def test_get_many(self, cache: ClusterValkeyCache):
        cache.set("a", 1)
        cache.set("b", 2)
        cache.set("c", 3)
    ...
    def test_get_many_unicode(self, cache: ClusterValkeyCache):
        cache.set("a", "1")
        cache.set("ب", "2")
        cache.set("c", "الف")
    ...
    def test_set_many(self, cache: ClusterValkeyCache):
        cache.set_many({"a{foo}": 1, "b{foo}": 2, "c{foo}": 3})
        res = cache.get_many(["a{foo}", "b{foo}", "c{foo}"])
    ...
    def test_delete(self, cache: ClusterValkeyCache):
        cache.set_many({"a{foo}": 1, "b{foo}": 2, "c{foo}": 3})
        res = cache.delete("a{foo}")
        assert bool(res) is True

        res = cache.get_many(["a{foo}", "b{foo}", "c{foo}"])
        assert res == {"b{foo}": 2, "c{foo}": 3}

        res = cache.delete("a{foo}")
        assert bool(res) is False

    def test_delete_return_value_type(self, cache: ClusterValkeyCache):
    ...
    def test_delete_many(self, cache: ClusterValkeyCache):
        cache.set_many({"a{foo}": 1, "b{foo}": 2, "c{foo}": 3})
        res = cache.delete_many(["a{foo}", "b{foo}"])
        assert bool(res) is True
    ...
    def test_delete_many_generator(self, cache: ClusterValkeyCache):
        cache.set_many({"a{foo}": 1, "b{foo}": 2, "c{foo}": 3})
        res = cache.delete_many(key for key in ["a{foo}", "b{foo}"])
        assert bool(res) is True
    ...
    def test_incr_no_timeout(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, herd.HerdClient):
            pytest.skip("HerdClient doesn't support incr")
    ...
    def test_incr_error(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, herd.HerdClient):
            pytest.skip("HerdClient doesn't support incr")
    ...
    def test_incr_ignore_check(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
    ...
    def test_get_set_bool(self, cache: ClusterValkeyCache):
        cache.set("bool", True)
        res = cache.get("bool")
    ...
    def test_decr(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, herd.HerdClient):
    ...
    def test_version(self, cache: ClusterValkeyCache):
    ...
    def test_incr_version(self, cache: ClusterValkeyCache):
        cache.set("keytest", 2)
    ...
    def test_ttl_incr_version_no_timeout(self, cache: ClusterValkeyCache):
    ...
    def test_close(self, cache: ClusterValkeyCache, settings: SettingsWrapper):
        settings.DJANGO_VALKEY_CLOSE_CONNECTION = True
        cache.set("f", "1")
        cache.close()

    def test_close_client(self, cache: ClusterValkeyCache, mocker: MockerFixture):
        mock = mocker.patch.object(cache.client, "close")
        cache.close()
        assert mock.called

    def test_ttl(self, cache: ClusterValkeyCache):
    ...
    def test_pttl(self, cache: ClusterValkeyCache):
        # Test pttl
        cache.set("foo", "bar", 10)
        ttl = cache.pttl("foo")
    ...
    def test_persist(self, cache: ClusterValkeyCache):
    ...
    def test_expire(self, cache: ClusterValkeyCache):
    ...
    def test_expire_with_default_timeout(self, cache: ClusterValkeyCache):
        cache.set("foo", "bar", timeout=None)
        assert cache.expire("foo", DEFAULT_TIMEOUT) is True
        assert cache.expire("not-existent-key", DEFAULT_TIMEOUT) is False

    def test_pexpire(self, cache: ClusterValkeyCache):
        cache.set("foo", "bar", timeout=None)
    ...
    def test_pexpire_with_default_timeout(self, cache: ClusterValkeyCache):
        cache.set("foo", "bar", timeout=None)
        assert cache.pexpire("foo", DEFAULT_TIMEOUT) is True
        assert cache.pexpire("not-existent-key", DEFAULT_TIMEOUT) is False

    def test_pexpire_at(self, cache: ClusterValkeyCache):
    ...
    def test_expire_at(self, cache: ClusterValkeyCache):
        # Test settings expiration time 1 hour ahead by datetime.
        cache.set("foo", "bar", timeout=None)
        expiration_time = datetime.datetime.now() + timedelta(hours=1)
    ...
    def test_lock(self, cache: ClusterValkeyCache):
    ...
    def test_lock_released_by_thread(self, cache: ClusterValkeyCache):
    ...
    def test_iter_keys(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
    ...
    def test_iter_keys_itersize(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support iter_keys")
    ...
    def test_iter_keys_generator(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support iter_keys")
    ...
    def test_primary_replica_switching(self, cache: ClusterValkeyCache):
        ...
        cache = cast(ClusterValkeyCache, caches["sample"])
        client = cache.client
    ...
    def test_primary_replica_switching_with_index(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support get_client")
    ...
    def test_touch_zero_timeout(self, cache: ClusterValkeyCache):
    ...
    def test_touch_positive_timeout(self, cache: ClusterValkeyCache):
        cache.set("test_key", 222, timeout=10)
    ...
    def test_touch_negative_timeout(self, cache: ClusterValkeyCache):
        cache.set("test_key", 222, timeout=10)
    ...
    def test_touch_missed_key(self, cache: ClusterValkeyCache):
        assert cache.touch("test_key_does_not_exist", 1) is False

    def test_touch_forever(self, cache: ClusterValkeyCache):
    ...
    def test_touch_forever_nonexistent(self, cache: ClusterValkeyCache):
        result = cache.touch("test_key_does_not_exist", None)
        assert result is False

    def test_touch_default_timeout(self, cache: ClusterValkeyCache):
        cache.set("test_key", "foo", timeout=1)
    ...
    def test_clear(self, cache: ClusterValkeyCache):
        cache.set("foo", "bar")
    ...
    def test_hset(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support get_client")
    ...
    def test_hdel(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support get_client")
    ...
    def test_hlen(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support get_client")
    ...
    def test_hkeys(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support get_client")
    ...
    def test_hexists(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support get_client")
    ...
    def test_scard(self, cache: ClusterValkeyCache):
    ...
    def test_interstore(self, cache: ClusterValkeyCache):
        if isinstance(cache.client, ShardClient):
            pytest.skip("ShardClient doesn't support get_client")
    ...
    def test_sismember_bytearray(self, cache: ClusterValkeyCache):
        if isinstance(cache.client._serializer, JSONSerializer):
    ...
    def test_sismember_memoryview(self, cache: ClusterValkeyCache):
    ...
    def test_sismember_complex(self, cache: ClusterValkeyCache):
        if isinstance(cache.client._serializer, (JSONSerializer, MSGPackSerializer)):
            pytest.skip(
    ...
    def test_sismember_list(self, cache: ClusterValkeyCache):
        cache.sadd("foo", [1, 2, 3])
        assert cache.sismember("foo", [1, 2, 3]) is True
        assert cache.sismember("foo", [1, 2, 4]) is False

    def test_sismember_tuple(self, cache: ClusterValkeyCache):
        cache.sadd("foo", (1, 2, 3))
        assert cache.sismember("foo", (1, 2, 3)) is True
        assert cache.sismember("foo", (1, 2, 4)) is False

    def test_sismember_set(self, cache: ClusterValkeyCache):
    ...
    def test_sismember_frozenset(self, cache: ClusterValkeyCache):
        if isinstance(cache.client._serializer, (MSGPackSerializer, JSONSerializer)):
    ...
    def test_spop_default_count(self, cache: ClusterValkeyCache):
        cache.sadd("foo", "bar1", "bar2")
        assert cache.spop("foo") in {"bar1", "bar2"}
        assert cache.smembers("foo") in [{"bar1"}, {"bar2"}]

    def test_spop(self, cache: ClusterValkeyCache):
        cache.sadd("foo", "bar1", "bar2")
        assert cache.spop("foo", 1) in [{"bar1"}, {"bar2"}]
        assert cache.smembers("foo") in [{"bar1"}, {"bar2"}]

    def test_srandmember_default_count(self, cache: ClusterValkeyCache):
        cache.sadd("foo", "bar1", "bar2")
    ...
    def test_sscan_iter_with_match(self, cache: ClusterValkeyCache):
        if cache.client._has_compression_enabled():
            pytest.skip(
    ...
    def test_smismember(self, cache: ClusterValkeyCache):
